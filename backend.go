...
func (gm *Game) move(cor1, cor2 string) bool { //move(x1 int, y1 int, x2 int, y2 int) bool {
	...
	if x1 < 0 || x1 > 7 || x2 < 0 || x2 > 7 || y1 < 0 || y1 > 7 || y2 < 0 || y2 > 7 || (x1 == x2 && y1 == y2) {
		...
		if gm.board[x1][y1].chess_type != "King" || gm.board[x2][y2].chess_type != "Rook" {
			return false
		}
	...
	if cur.chess_type == "Rook" {
		...
		if abs(x1-x2) > 1 || abs(y1-y2) > 1 {
			if gm.board[x2][y2].chess_type == "Rook" && !cur.moved && !gm.board[x2][y2].moved && gm.board[x2][y2].party == cur.party {
				isBlocked := false
				for i := ternary(y1 < y2, 1, -1).(int); y1+i != y2; i += ternary(y1 < y2, 1, -1).(int) {
					if gm.board[x1][y1+i].chess_type != "blank" {
						isBlocked = true
				...
				if isBlocked {
					fmt.Printf("isBlocked\n")
					return false
				}
				if y1 < y2 {
					gm.board[x1][y1+2] = cur
					gm.board[x1][y1+2].moved = true
					gm.board[x1][y1+1] = gm.board[x2][y2]
					gm.board[x1][y1+1].moved = true
					cur = Chess{
						chess_type: "blank",
						party:      3,
						moved:      true,
